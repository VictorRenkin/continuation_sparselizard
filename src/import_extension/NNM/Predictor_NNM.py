...
class AbstractPredictor(ABC):
    ...
    @abstractmethod
    def set_initial_prediction(self, elasticity, tan_w=0, tan_mu=0, h=5e-5):
        pass

    @abstractmethod
    def prediction_direction(self, PreviousPoint, PhaseCondition, elasticity, field_u, vec_u, PHYSREG_U, h=5e-5) :
        pass

    @abstractmethod
    def predict(self, PreviousPoint, PhaseCondition, elasticity, field_u, PHYSREG_U, h=5e-5):
        """
        Predict the point .
...
# class PredictorSecant(AbstractPredictor) :
    ...
    # def set_initial_prediction_tan_w(self, elasticity, tan_w=0, tan_mu=0, h=5e-5) :
...
class PredictorTangent(AbstractPredictor):
    ...
    def predict(self, PreviousPoint, PhaseCondition, elasticity, field_u, PHYSREG_U, h=5e-5) :
        ...
        tan_norm = (self.tan_u.norm() + self.tan_w**2 + self.tan_mu**2)**0.5
        u_pred = self.length_s * self.tan_u/tan_norm + prev_point['u']
        f_pred = self.length_s * self.tan_w/tan_norm +  prev_point['freq']

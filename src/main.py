...
# in this code the z is instinfy like a vector, and the u is a field
...
f_start = 158 ;fd_min = 155; fd_max = 168
freq_step_first_iteraton = 5*1e-3
max_iter = 10

clk = sp.wallclock()
vec_delta_u_1, Jac_1, _ = se.get_newthon_raphson_without_predictor(f_start, elasticity, u, vol)
u.setdata(vol, vec_delta_u_1)
um = sp.norm(u.harmonic(2)).max(loadpoint,3)[0]
cd.add_data_to_csv(um, f_start, '../data/FRF/NLFRs.csv')
vd.real_time_plot_data_FRF('../figures/linear_FRF.pdf', '../data/FRF/linear_FRF.csv')

f_2 = f_start + freq_step_first_iteraton
z2, Jac_2, b_2 = se.get_newthon_raphson_without_predictor(f_2, elasticity, u, vol)
u.setdata(vol, z2)
um = sp.norm(u.harmonic(2)).max(loadpoint,3)[0]
...
while f_2 < fd_max and f_2 > fd_min :
    ...
    delta_vec_u_pred, delta_w_pred = se.compute_tan_predictor(length_s, tan_u, tan_w)
    vec_u_pred = delta_vec_u_pred + z2
    u.setdata(vol, vec_u_pred)
    f_pred = delta_w_pred + f_2
    sp.setfundamentalfrequency(f_pred)
    if delta_w_pred + f_2 < fd_min or delta_w_pred + f_2> fd_max:
        break
    max_u_point, freq, iter = se.get_predictor_corrector_NewtonSolve(elasticity, vol, u, f_2, delta_vec_u_pred, delta_w_pred, tan_u, tan_w, Jac_2, b_2, tol = 1e-6, max_iter = max_iter)
    if iter == max_iter:
        length_s = length_s/2
    else:
        z1 = z2 ; f_1 = f_2

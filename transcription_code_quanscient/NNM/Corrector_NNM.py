...
class CorrectorPseudoArcLength(AbstractCorrector):
    ...
    def correct_step(self, elasticity, PHYSREG_U, HARMONIC_MEASURED, field_u, 
        ...
        while iter_new < self.MAX_ITER:
            ...
            qs.printonrank(0,f"Iteration {iter_new}: Residual max G: {(residue_G * residue_G)**0.5:.2e}, frequence: {f_k}")
            if (residue_G * residue_G)**0.5 < self.TOL and fct_g < self.TOL and phase_condition < self.TOL:
                break
        ...
        return u_k, f_k, mu_k, iter_new, residue_G, Jac_k, grad_mu_G
...
class CorrectorAmplitude(AbstractCorrector):
    ...
    def corrector_condition(self, u_k, desired_amplitude):
        ...
        """
        return u_k * u_k - desired_amplitude**2
    ...
    def correct_step(self, elasticity, PHYSREG_U, HARMONIC_MEASURED, u, 
        ...
        while iter_new < self.MAX_ITER:
            ...
            grad_w_g = 0
            grad_mu_g = 0
            qs.printonrank(0,f"Iteration {iter_new}: Residual max G: {(residue_G * residue_G)**0.5:.2e}, frequence: {f_k}, fct_g :{fct_g}")
            if (residue_G * residue_G)**0.5 < self.TOL  and fct_g > 0 and phase_condition < self.TOL:
                break

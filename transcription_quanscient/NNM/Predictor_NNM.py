...
class AbstractPredictor(ABC):
    ...
    def set_predictor(self, field_u, PHYSERG_U, PhaseCondition, f_pred, u_pred, mu_pred, tan_u=None, tan_w=None, tan_mu=None):
        ...
        qs.setfundamentalfrequency(f_pred)
...
class PredictorTangent(AbstractPredictor):
    ...
    def set_initial_prediction(self, elasticity, tan_w=1, tan_mu=0) :
        
        tan_u = qs.vec(elasticity)
        densemat_tan_u = qs.densemat(tan_u.size(), 1, 1)
    ...
    def predict(self, PreviousPoint, PhaseCondition, elasticity, field_u, PHYSREG_U) :
        ...
        tan_norm = (self.tan_u * self.tan_u+ self.tan_w**2 + self.tan_mu**2)**0.5
...
class PredictorSecant(AbstractPredictor):
    ...
    def prediction_direction(self, PreviousPoint, PhaseCondition, elasticity, field_u, vec_u, PHYSREG_U, clk_generate, clk_solver) :
        ...
        if len(PreviousPoint) < 2 : 
            ...
            tan_mu = (prev_point['mu'] - prev_point_1['mu']) 
            self.tan_u = tan_u
            self.tan_w = tan_w
            self.tan_mu = tan_mu
            return tan_u, tan_w, tan_mu
